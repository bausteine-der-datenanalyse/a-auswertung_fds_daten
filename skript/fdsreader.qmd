...
Um Simulationsdaten, die mit FDS berechnet wurden, in Python zu analysieren, 
hat die Forschungsgruppe von Prof. Lukas Arnold das Python-Modul **fdsreader** 
entwickelt. Dieses Modul ermöglicht es, die meisten von FDS erzeugten 
Datenformate einzulesen und in Python-Datenstrukturen zu überführen.

Das Modul ist kostenlos und als Open Source verfügbar. Der Quellcode ist auf 
GitHub zu finden unter: [FireDynamics/fdsreader](https://github.com/FireDynamics/fdsreader). 
...
Um die grundlegende Nutzung des **fdsreader**-Moduls zu erlernen, werden wir 
uns ein einfaches FDS-Szenario ansehen. Zuerst importieren wir das Modul:

```{python}
import fdsreader
```

Da wir die Daten auch plotten werden, importieren wir zusätzlich `matplotlib`:
...
Als nächstes muss der Reader auf das Verzeichnis verwiesen werden, das die 
...
Das `Simulation`-Objekt `sim` enthält nun alle Informationen und Daten zu 
den Simulationsausgaben:

```{python}
sim
```

Die Variable `sim` enthält Informationen über das Mesh (`MESH`), vier 
Schnitte (`SLCF`) und vier Punktmessungen (`DEVC`). Zusätzlich gibt es eine 
Zeitspalte, die als vierte Messgröße hinzugefügt wurde, obwohl in der 
FDS-Eingabedatei ursprünglich nur drei Geräte definiert waren.
